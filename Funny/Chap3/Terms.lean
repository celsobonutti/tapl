...
def List.max
  ...
  theorem succ_x_not_number_implies_x_not_number : ∀ {x : T}, ¬is_number (T.succ x) → ¬is_number x := by
    intro t not_number
    by_contra h
    exact not_number (is_number_succ _ h)
  ...
  open T in
  inductive BigStep : Rel T T where
    | value : ∀ {t}, {_ : is_normal_form t} → BigStep t t
    | if_true : ∀ {t₁ t₂ t₃ v₂}, BigStep t₁ true → BigStep t₂ v₂ → BigStep (if_then_else t₁ t₂ t₃) v₂
    | if_false : ∀ {t₁ t₂ t₃ v₃}, BigStep t₁ false → BigStep t₃ v₃ → BigStep (if_then_else t₁ t₂ t₃) v₃
    | succ : ∀ {t₁ nv₁}, {_ : is_number nv₁} → BigStep t₁ nv₁ → BigStep (succ t₁) (succ nv₁)
    | pred_zero : ∀ {t₁}, BigStep t₁ zero → BigStep (pred t₁) zero
    | pred_succ : ∀ {t₁ nv₁}, {_ : is_number nv₁} → BigStep t₁ (succ nv₁) → BigStep (pred t₁) nv₁
    | is_zero_zero : ∀ {t₁}, BigStep t₁ zero → BigStep (is_zero t₁) true
    | is_zero_succ : ∀ {t₁ nv₁}, {_ : is_number nv₁} → BigStep t₁ (succ nv₁) → BigStep (is_zero t₁) false
